<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            Deal # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            Deal
        </v-card-title >        
        ...
        <v-card-text>
            <Number v-if="editMode" label="DealId" v-model="value.dealId" :editMode="editMode" :inputUI="''"/>
            ...
            <Number label="OfferId" v-model="value.offerId" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/deals'), this.value)
                        } else {
